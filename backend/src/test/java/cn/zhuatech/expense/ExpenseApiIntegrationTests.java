...
class ExpenseApiIntegrationTests {
     ...
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
    @Test void adminCanAssessExpenseCompliance() throws Exception {
        mvc.perform(post("/api/admin/expense-compliance").with(httpBasic("admin", "admin123"))
            .contentType(MediaType.APPLICATION_JSON)
            .content("{\"amount\":12800.50,\"receiptCoverage\":90,\"policyOverage\":8,\"duplicateMatches\":2,\"costCenterValid\":true,\"preApprovalRequired\":true,\"preApproved\":false}"))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.data.riskScore").value(86))
